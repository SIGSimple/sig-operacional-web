...
class upload {
     ...
     * @param  resource $image Optional Original image
     * @return resource Container image
     */
    function create_image($x, $y, $image) {
        ...
        if (!empty($this->image_background_color)) {
            ...
            $background_color = imagecolorallocate($tmp, $red, $green, $blue);
     ...
     *
     * @access private
     * @param  resource $source Container image
     * @param  resource $dest   Destination image
     * @return resource Destination image
     */
    function transfer_image($source, $dest) {

        imagedestroy($dest);     
        $dest=imagecreatetruecolor($this->image_dst_x, $this->image_dst_y);
        imagecopy($dest,$source,0,0,0,0,$this->image_dst_x,$this->image_dst_y);
        imagedestroy($source);
            ...
            if ($image_manipulation) {
                    ...
                    switch($this->file_src_mime) {
                        ...
                        case 'image/gif':
                            if (!function_exists('imagecreatefromgif')) {
                                $this->processed = false;
                                $this->error = _("Error ao criar o arquivo GIF");
                            } else {
                        ...
                        default:
                            $this->processed = false;
                            $this->error = _("Só é permitido upload de imagem");
                    }
                } else {
                    $this->processed = false;
                ...
                if ($this->processed && $image_src) {
                    ...
                    if ($this->image_resize) {
                            ...
                            if ((!$this->image_ratio_no_zoom_in && !$this->image_ratio_no_zoom_out)
                                ...
                                if ($this->image_ratio_crop) {
                                    ...
                                    if (($this->image_src_x/$this->image_x) < ($this->image_src_y/$this->image_y)) {                                 
                                        ...
                                        $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;' . _("ratio_fill_x") . '         : ' . $ratio_crop['x'] . ' (' . $ratio_crop['l'] . ';' . $ratio_crop['r'] . ')<br />';
                                        if (is_null($this->image_crop)) $this->image_crop = array(0, 0, 0, 0);
                                    } else {                 
                                        $this->image_dst_x = $this->image_x;
                                        $this->image_dst_y = intval($this->image_src_y*($this->image_x / $this->image_src_x));
                                        ...
                                        $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;' . _("ratio_fill_y") . '         : ' . $ratio_crop['y'] . ' (' . $ratio_crop['t'] . ';' . $ratio_crop['b'] . ')<br />';
                                        if (is_null($this->image_crop)) $this->image_crop = array(0, 0, 0, 0);
                                    }                                    
                                } else {
                                    if (($this->image_src_x/$this->image_x) > ($this->image_src_y/$this->image_y)) {
                        ...
                        // we eventually fill the receiving image with the background color
                        if (!empty($this->image_background_color)) {
                            $this->log .= '- ' . _("set background color") . '<br />';
                            sscanf($this->image_background_color, "#%2x%2x%2x", $red, $green, $blue);
                            $background_color = imagecolorallocate($image_src, $red, $green, $blue);
                            imagefill($image_src, 0, 0, $background_color);
                            imagecolortransparent($image_dst, imagecolorexact($image_dst, 0, 0, 0));
                        }
                    ...
                    if ($gd_version >= 2 && !imageistruecolor($image_dst)) {
                        $transparent_color = imagecolortransparent($image_dst);
                        if ($transparent_color >= 0) {
                            $rgb = imagecolorsforindex($image_dst, $transparent_color);
                            $transparent_color = ($rgb['red'] << 16) | ($rgb['green'] << 8) | $rgb['blue'];
                            imagecolortransparent($image_dst, imagecolorallocate($image_dst, 0, 0, 0));
                        }
                        ...
                        imagecopy($true_color, $image_dst, 0, 0, 0, 0, $this->image_src_x, $this->image_src_y);
                        imagedestroy($image_dst);
                        $image_dst = $true_color;
                        if ($transparent_color >= 0) {
                            imagealphablending($image_dst, false);
                            imagesavealpha($image_dst, true);
                            for ($x = 0; $x < $this->image_src_x; $x++) {
                                for ($y = 0; $y < $this->image_src_y; $y++) {
                                    if (imagecolorat($image_dst, $x, $y) == $transparent_color)
                                        imagesetpixel($image_dst, $x, $y, 127 << 24);
                                }
                            }
                        }
                    }
                    ...
                    if ($gd_version >= 2 && (!empty($this->image_crop) || !is_null($ratio_crop))) {
                        ...
                        $tmp = $this->create_image($this->image_dst_x, $this->image_dst_y, $image_dst);
                        ...
                        $image_dst = $this->transfer_image($tmp, $image_dst);
                    }
                    ...
                    if ($gd_version >= 2 && is_numeric($this->image_rotate)) {
                        ...
                        if ($this->image_rotate != 0) {
                            if ($this->image_rotate == 90 || $this->image_rotate == 270) {
                                $tmp = $this->create_image($this->image_dst_y, $this->image_dst_x, $image_dst);
                            } else {
                                $tmp = $this->create_image($this->image_dst_x, $this->image_dst_y, $image_dst);
                            }
